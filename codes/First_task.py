...
import os
from tensorflow.python.keras.layers import Input, Conv1D, AveragePooling1D, GlobalAveragePooling1D, \
...
# Building the model
def build_model(windows=7, dense_blocks=5, layers=2, filters=96,
...
if __name__ == '__main__':
    ...
    train_pos_seqs = np.array(
        read_fasta('/home/iEnhancer-DS/data/layer1/train_enhancers.fa'))
    train_neg_seqs = np.array(
        read_fasta('/home/iEnhancer-DS/data/layer1/train_nonenhancers.fa'))
    ...
    # train = train_properties_code

    train_label = np.array([1] * 1484 + [0] * 1484).astype(np.float32)
    train_label = to_categorical(train_label, num_classes=2)

    # Read the testing set
    test_pos_seqs = np.array(
        read_fasta('/home/iEnhancer-DS/data/layer1/test_enhancers.fa'))
    test_neg_seqs = np.array(
        read_fasta('/home/iEnhancer-DS/data/layer1/test_nonenhancers.fa'))
    ...
    for k in range(10):
        ...
        for fold_count, (train_index, val_index) in enumerate(k_fold.split(train)):
            ...
            history = model.fit(x=trains, y=trains_label, validation_data=(val, val_label), epochs=EPOCHS,
                                ...
                                callbacks=[EarlyStopping(monitor='val_loss', patience=60, mode='auto')],
                                verbose=1)  # callbacks回调，将数据传给history

            with open('/home/iEnhancer-DS/one_log_history.txt', 'w') as f:
            ...
            model.save('/home/iEnhancer-DS/models/one/one_enhancer_model_' + str(
                fold_count) + '.h5')

            del model

            model = load_model(
                '/home/iEnhancer-DS/models/one/one_enhancer_model_' + str(
                    fold_count) + '.h5')
    ...
    # 保存并显示图像
    plt.savefig('/home/iEnhancer-DS/images/ROC_Curve_of_First_Layer.jpg',
